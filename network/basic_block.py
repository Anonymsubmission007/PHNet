...
class conv_layer(nn.Module):
    def __init__ (self, dim, res_ratio):
        super(conv_layer, self).__init__()
        self.network = []
        for i in range(3):
            if res_ratio < 2**i+ 2**(i-1):  # isotropic
                self.network.append(Encoder(in_chns=dim[i],out_chns=dim[i+1],k=3,p=1))
                if i < 2:
                    self.network.append(nn.MaxPool3d(kernel_size=2,stride=2))
            else:  # anisotropic
                self.network.append(Encoder(in_chns=dim[i],out_chns=dim[i+1],k=(3,3,1),p=(1,1,0)))
                if i < 2:
