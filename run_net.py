...
def get_xforms(mode="train", keys=("image", "label"), spatial_size=(192, 192, 16)):
    """returns a composed transform for train/val/infer."""

    xforms = [
        LoadImaged(keys, ensure_channel_first=True, image_only=True),
        Orientationd(keys, axcodes="LPS"),
        Spacingd(keys, pixdim=(0.74, 0.74, 5.0), mode=("bilinear", "nearest")[: len(keys)]),
        ScaleIntensityRanged(keys[0], a_min=-1000.0, a_max=500.0, b_min=0.0, b_max=1.0, clip=True),
    ]
    if mode == "train":
        xforms.extend(
            [
                SpatialPadd(keys, spatial_size, mode="reflect"),  # ensure at least 192x192
                RandAffined(
                    keys,
                    prob=0.15,
                ...
                RandCropByPosNegLabeld(keys, label_key=keys[1], spatial_size=spatial_size, num_samples=3),
                SpatialPadd(keys, spatial_size=spatial_size, mode="reflect"),
...
def train(args, config, data_folder="."):
    ...
    train_transforms = get_xforms("train", keys, config['patch_size'])
    ...
    max_epochs, lr = args.max_epochs, args.lr
    ...
    evaluator = monai.engines.SupervisedEvaluator(
        ...
        network=net,
        inferer=get_inferer(config['patch_size']),
    ...
    train_handlers = [
        ValidationHandler(validator=evaluator, interval=args.interval, epoch_level=True),
...
def infer(args, config, data_folder=".", prediction_folder="output"):
    """
    run inference, the output folder will default be "./output"
    """
    model_folder=args.model_folder
    ...
    for ckpt in ckpts:
        ...
        curr_prediction_folder = os.path.join(prediction_folder,ckpt[:-3])
        ...
        inferer = get_inferer(config['patch_size'])
        saver = monai.transforms.SaveImage(output_dir=curr_prediction_folder, mode="nearest", resample=True)
        with torch.no_grad():
        ...
        submission_dir = os.path.join(curr_prediction_folder, "to_submit")
        if not os.path.exists(submission_dir):
            os.makedirs(submission_dir)
        files = glob.glob(os.path.join(curr_prediction_folder, "volume*", "*.nii.gz"))
        for f in files:
        ...
        align_shape(submission_dir,data_folder) # in case the shape does not match between image and segmentation mask
...
if __name__ == "__main__":
    ...
    parser.add_argument("--data_folder", default="", type=str, help="training/validation data folder")
    parser.add_argument("--model_folder", default="runs", type=str, help="model folder")
    parser.add_argument("--prediction_folder", default="output", type=str, help="prediction output folder")
    parser.add_argument("--batch_size", default=1, type=int, help="train batch size")
    parser.add_argument("--interval", default=4, type=int, help="validation interval")
    parser.add_argument("--lr", default=8e-5, type=float, help="learning rate")
    parser.add_argument("--max_epochs", default=450, type=int, help="maximum training epochs")
    args = parser.parse_args()

    monai.config.print_config()
    monai.utils.set_determinism(seed=0)
    logging.basicConfig(stream=sys.stdout, level=logging.INFO)
    config = get_covid_config()

    if args.mode == "train":
        data_folder = args.data_folder or os.path.join("COVID-19-20_v2", "Train")
        train(args, config, data_folder=data_folder)
    elif args.mode == "infer":
        data_folder = args.data_folder or os.path.join("COVID-19-20_v2", "Validation")
        infer(args, config, data_folder=data_folder)
    else:
